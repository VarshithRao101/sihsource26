...
| **.shp export** | `api.py` `/api/runs/{id}/export?format=shp` → `modules/04_backend/exports.py` | Verified live: a **GIS delivery package**, not a geometry dump. Zip of the extent layer (`.shp/.shx/.dbf/.prj/.cpg`, depth-banded), a settlements point layer carrying arrival time, depth, velocity, hazard class and damage, plus `settlements.csv`, `roads_cut.csv`, `hydrograph.csv`, `meta.json`, `impact.json`, `uncertainty.json` and a `README.txt` that states the headline results and **every caveat the run recorded** — a DBF field name cannot hold a paragraph, so the paragraph ships beside it | ✅ |
| **.kml export** | same, `format=kml` | Verified live, well-formed XML, opens in Google Earth. Hand-written rather than driver-written: polygons **styled by depth band**, the barrier as a placemark, every affected settlement as a hazard-coloured placemark whose balloon carries its arrival time, depth, velocity and how its population was obtained, the roads cut, and a Document description that puts the headline numbers and the limitations in front of the reader before the geometry | ✅ |
| GeoJSON (bonus) | same, `format=geojson` | Verified live. The contract file in the run folder stays minimal; the export adds per-feature depth bands and provenance and a top-level `run` and `caveats` block as GeoJSON foreign members, which every reader ignores safely | ✅ |
| **Every export carries its caveats** | `exports.caveats()` | Assembled from what the run folder actually recorded, so a caveat that applies cannot be dropped and one that does not apply is never emitted as boilerplate. A knife-edge impounded volume, a class-default population, an unvalidated extent and a barrier-less mode each say so **in the file that leaves the building** | ✅ |
